        ...
        for iteration in range(1, self.n_epochs + 1):
            ...
            for batch in batch_generator(self.batch_size, data):
                ...
                for sample in batch:
                    ...
                    accum_delta_W = accum_delta_W + delta_W
                    accum_delta_b = accum_delta_b + delta_b
                    accum_delta_c = accum_delta_c + delta_c
                self.W = self.W + self.learning_rate * (accum_delta_W / self.batch_size)
                self.b = self.b + self.learning_rate * (accum_delta_b / self.batch_size)
                self.c = self.c + self.learning_rate * (accum_delta_c / self.batch_size)
            if self.verbose:
            ...
            for batch_data, batch_labels in batch_generator(self.batch_size, data, labels):
                ...
                for sample, label in zip(batch_data, batch_labels):
                    ...
                    for layer in range(len(self.unsupervised_dbn.rbm_layers) + 1):
                        accum_delta_W[layer] = accum_delta_W[layer] + delta_W[layer]
                        accum_delta_bias[layer] = accum_delta_bias[layer] + delta_bias[layer]
                    if self.verbose:
...
class SupervisedDBNClassification(NumPyAbstractSupervisedDBN, ClassifierMixin):
    ...
    def _compute_output_units(self, vector_visible_units):
        ...
        exp_scores = np.exp(scores.astype(np.float))
    ...
    def _compute_output_units_matrix(self, matrix_visible_units):
        ...
        exp_scores = np.exp(matrix_scores.astype(np.float))
